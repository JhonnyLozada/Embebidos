...
int posiciones1=0;
for(int h =39;h<80;h++){
float    distancia1=sqrt(pow((datos[h][0]-promedios2[0]),2)+
             pow((datos[h][1]-promedios2[1]),2)+
             pow((datos[h][2]-promedios2[2]),2)+
             pow((datos[h][3]-promedios2[3]),2));
             Serial.println(distancia1);
             if((distancia1/maximos2[0])>=minim &&(distancia1/maximos2[0])<=maxim){
             distancias1[h]=distancia1;
              for(int m=0;m<5;m++){
                type2[posiciones1][m]=datos[posiciones1][m];
              }
              posiciones1++;
             }
...
float maximo1=distancias1[0];

for(int h=40;h<80;h++){
  if(distancias1[h]>maximo1){
    maximo1=distancias1[h];
  }
}
int contador1=0;
for(int f=40;f<80;f++){
  if(((distancias1[f]/maximo1)>minim )&& ((distancias1[f]/maximo1)<maxim)){
    type2[contador1][0]=datos[f][0];
    type2[contador1][1]=datos[f][1];
    type2[contador1][2]=datos[f][2];
    type2[contador1][3]=datos[f][3];
    type2[contador1][4]=datos[f][4];
    contador1++;
  }
}

for(int f=0;f<=posiciones1;f++){
    for(int s=0;s<5;s++){

      Serial.print(type2[f][s]);
      Serial.print(" ");
    }
    Serial.println(" ");
}
