...
float matriz_d [110][5]; // datos de desecho
...
void setup(){
  ...
  for(;etiqueta<5;etiqueta++){ // ciclo que recorre por etiqueta
      for(;col<4;col++){ // ciclo que recorre por columna
        for(fil=i;fil<33*etiqueta;fil++){ // ciclo que recorre por filas
    ...
    i=33*etiqueta;  // variable que que cuenta determina el numero de datos por etiqueta 
 } 
...
void loop(){
   ...
   */
   ...
   if(on==0){
   for(;fil<110;fil++){ // ciclo que recorre por cada fila de matriz de mayor volumen
